...
class DAE(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim, activation, lambda1, lambda2, lambda3, lambda4, lambda5, lambda6, q):
        """ Constructor
    ...
    def weighted_quantile_loss(self, predicted, target):
            loss = torch.mean(2 * sum((self.q*torch.max((target - predicted), torch.zeros(np.shape(target-predicted))))+((1-self.q)*torch.max((predicted - target), torch.zeros(np.shape(predicted-target)))))/torch.abs(torch.sum(target)))
            #print(loss.shape)
            return loss


    def w_loss(self, predicted, target):
        predicted, encoder_output = predicted
        encoder_output = torch.split(encoder_output, self.latent_dim, dim=1)
        return (self.weighted_quantile_loss(target[0], predicted[0]) + 
                self.weighted_quantile_loss(target[1], predicted[1]) + self.weighted_quantile_loss(encoder_output[0], encoder_output[1]))
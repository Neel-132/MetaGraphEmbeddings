...
# File: AEME.py
# Author: Shashwat Jha and Neelabha Banerjee
...
class AEME():
    ...
    def __init__(self, mode="CAE", input_dim=300, latent_dim=100, activation="leaky_relu", lambda1=1, lambda2=1, lambda3 = 1, lambda4=1, lambda5=1, lambda6=1, q = 0.9):
        ...
        if mode == "DAE":
            self.ae = DAE(input_dim, latent_dim, activation, lambda1, lambda2, lambda3, lambda4, lambda5, lambda6, q)
        elif mode == "CAE":
            self.ae = CAE(input_dim, latent_dim, activation, lambda1, lambda2, lambda3, q)
        elif mode == "AAE":
            self.ae = AAE(input_dim, latent_dim, activation, lambda1, lambda2, q)
    ...
    def prepare_input(self, vocab, x_train1, x_train2, batch_size=157, masking_noise_factor=0.05): 
        ...
        #print(x_train1_noisy.shape)
        #print(x_train2_noisy.shape)
        ...
        if self.mode == "DAE": 
            for step in range(1, epochs+1):
                ...
                for batch_data in tensor_dataset:
                    optimizer.zero_grad()
                    x_train1_noisy, x_train2_noisy, x_train1, x_train2, _ = tuple(t.to(self.device) for t in batch_data)
                    output, bottleneck = self.ae(x_train1_noisy, x_train2_noisy)
                    loss = self.ae.w_loss([output, bottleneck], [x_train1, x_train2])
                    loss.backward()
                ...
                if len(training_loss) > 2:
                  if epoch_loss < training_loss[-2] and epochs == 500:
            ...
            for step in range(1, epochs+1):
                ...
                for batch_data in tensor_dataset:
                    optimizer.zero_grad()
                    x_train1_noisy, x_train2_noisy, x_train1, x_train2, _ = tuple(t.to(self.device) for t in batch_data)
                    output, _ = self.ae(x_train1_noisy, x_train2_noisy)
                    loss = self.ae.w_loss(output, [x_train1, x_train2])
                    loss.backward()
                    epoch_loss = epoch_loss + loss.item() 
                    optimizer.step()
                epoch_loss = epoch_loss/len(tensor_dataset)
    ...
    def predict(self, tensor_dataset, model_checkpoint):
        ...
        embedding_dict = {}
        for batch_data in tensor_dataset:
            _, _, x_train1, x_train2, nodes = tuple(t.to(self.device) for t in batch_data)
            nodes = self.label_encoder.inverse_transform(nodes.to('cpu')).tolist()
            
            with torch.no_grad():
                _, bottleneck = self.ae(x_train1, x_train2)
                bottleneck = torch.split(bottleneck, 1, dim=0)
                for word, vec in list(zip(nodes, bottleneck)):

...
class CAE(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim, activation, lambda1, lambda2, lambda3, q):
        """ Constructor
        ...
        self.decoder1 = nn.Linear(in_features= 2 * latent_dim, out_features=input_dim)
        nn.init.normal_(self.decoder1.weight, mean=0.0, std=0.01)
        nn.init.zeros_(self.decoder1.bias)
        
        self.decoder2 = nn.Linear(in_features=2 * latent_dim, out_features=input_dim)
    ...
    def forward(self, x1, x2):
        ...
        bottleneck = functional.normalize(torch.cat((x1, x2), dim=1), dim=1)
    ...
    def weighted_quantile_loss(self, predicted, target):
        diff = target - predicted
        zeros = torch.zeros(np.shape(diff))
        loss = 2 * torch.sum(self.q*torch.max(diff, zeros)+(1-self.q)*torch.max((-diff), zeros))/torch.sum(torch.abs(target))
        #print(loss.shape)
        return loss


    def w_loss(self, predicted, target):
        return (self.weighted_quantile_loss(target[0], predicted[0]) + 
                self.weighted_quantile_loss(target[1], predicted[1]))

    def weighted_quantile_loss_1(self, output, target):
        p = torch.tensor(output) - torch.tensor(target)
        loss = 0
        for e in p:
            loss += max(self.q * e, (self.q - 1) * e)
        return 2 * loss/torch.abs(torch.sum(target))
